---
name: Lois Andison
action: will comment on
website: http://loisandison.com
location: Toronto
topic1: language as artistic
topic2: medium
edition: 2017
---

A prolific artist based in Toronto, Lois’ works are manifested in print, photo and video, and sculpture and installation. Type and letter forms appear frequently in her practice, and their (re)recombination through illumination and clever custom mechanics opens up new ways of thinking about language and meaning. Lois is represented by [Olga Korper Gallery](http://www.olgakorpergallery.com/){:target='_blank' rel='noopener'} (Toronto) and [Art Mûr](http://artmur.com/){:target='_blank' rel='noopener'}, (Montreal), and an Associate Professor in the [Fine Arts Department](https://uwaterloo.ca/fine-arts/){:target='_blank' rel='noopener'} of the University of Waterloo. Lois is a special guest co-presented by [InterAccess](http://interaccess.org/){:target='_blank' rel='noopener'}.
